...
export default function LandingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!webhookUrl) {
      alert('系統還沒有設定發信 Webhook 網址，請先完成 GAS 部署與 .env.local 的設定。');
      setStatus('error');
      return;
    }

    try {
      // 由於 Google Apps Script Web App 常有 CORS 限制，這裡使用 no-cors 模式
      await fetch(webhookUrl, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ email, type: quoteType }),
        mode: 'no-cors' 
      });

      // no-cors 收不到詳細 Response，且不會拋錯，因此假設執行到這即為成功
      setStatus('success');
      setEmail('');
    ...
    <div className="py-24 px-6 max-w-3xl mx-auto animate-in zoom-in-95 duration-700">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <label className={`cursor-pointer border-2 p-4 text-center transition-all duration-300 ${quoteType === 'commercial' ? 'bg-amber-500/10 border-amber-500/50 rounded-organic-2 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : 'bg-stone-950/50 border-stone-800 rounded-organic-2 hover:border-amber-500/30 text-stone-400'}`}>
                <input type="radio" name="quote_type" value="commercial" className="hidden" checked={quoteType === 'commercial'} onChange={() => setQuoteType('commercial')} disabled={status === 'loading'} />
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                <Mail className={`h-5 w-5 ${status === 'success' ? 'text-green-500' : 'text-stone-500'}`} />
              </div>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} disabled={status === 'loading' || status === 'success'} required placeholder="name@example.com" className="w-full pl-11 pr-5 py-4 bg-stone-950/70 border-2 border-stone-800 rounded-organic-3 text-stone-100 placeholder-stone-500 focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 focus:outline-none text-base transition-all shadow-inner disabled:opacity-50" />
            </div>
          </div>

          <button type="submit" disabled={status === 'loading' || status === 'success'} className={`w-full py-4 text-stone-950 rounded-organic-2 font-black border-2 transition-all duration-300 text-lg mt-4 font-serif flex justify-center items-center gap-2 ${status === 'success' ? 'bg-green-500 border-green-400 pointer-events-none' : 'bg-amber-500 border-amber-400 hover:bg-amber-400 shadow-[0_0_20px_rgba(217,119,6,0.2)] hover:shadow-[0_0_30px_rgba(217,119,6,0.4)]'}`}>
            {status === 'loading' && <div className="w-5 h-5 border-2 border-stone-950 border-t-transparent rounded-full animate-spin"></div>}
            {status === 'loading' ? '發送中...' : status === 'success' ? '✨ 報價單已發送成功！' : '立即索取報價單'}
          </button>
        </form>
      ...
      <div className="mt-20 max-w-4xl mx-auto bg-stone-900/40 backdrop-blur-sm border-2 border-stone-800 rounded-organic-2 p-10 md:p-12 relative overflow-hidden group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
          ...
          {/* Instagram */}
          <a href="https://www.instagram.com/lacozzi_1314/" target="_blank" rel="noopener noreferrer" className="bg-stone-950/60 border-2 border-stone-800 hover:border-amber-500/60 p-6 rounded-organic-2 transition-all duration-300 hover:-translate-y-1 group/card flex flex-col justify-center items-start">
          ...
          {/* Facebook */}
          <a href="https://www.facebook.com/LaCozzi" target="_blank" rel="noopener noreferrer" className="bg-stone-950/60 border-2 border-stone-800 hover:border-amber-500/60 p-6 rounded-organic transition-all duration-300 hover:-translate-y-1 group/card flex flex-col justify-center items-start">
