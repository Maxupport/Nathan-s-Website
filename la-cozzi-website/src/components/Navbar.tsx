  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-stone-950/90 backdrop-blur-xl border-b-2 border-stone-800">
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-4 flex justify-between items-center relative">
        <div className="flex items-center space-x-3 relative group cursor-pointer" onClick={() => handleTabClick('landing')}>
          {/* 仿 Logo 的手繪圓形 */}
        ...
        <div className="hidden md:flex items-center space-x-4 lg:space-x-8 text-xs lg:text-sm font-bold text-stone-400 font-sans tracking-wide">
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => handleTabClick(tab.id)} className={`hover:text-amber-400 transition-colors duration-300 relative py-1 whitespace-nowrap ${activeTab === tab.id ? 'text-amber-400' : ''}`}>
              {tab.label}
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-stone-950/95 backdrop-blur-xl border-b border-stone-800 p-6 flex flex-col space-y-4 shadow-xl z-40 max-h-[80vh] overflow-y-auto">
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => handleTabClick(tab.id)} className={`text-lg font-bold font-sans tracking-wide text-left py-3 px-4 rounded-xl transition-colors ${activeTab === tab.id ? 'bg-amber-900/30 text-amber-400 border border-amber-500/20' : 'text-stone-300 hover:bg-stone-800/50'}`}>
              {tab.label}
            </button>
          ))}
